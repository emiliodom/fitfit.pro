...
test.describe("mobile app feel", () => {
    ...
    test("tabs switch on mobile", async ({ page }) => {
        ...
        await expect(page.getByRole("heading", { name: /Running Lab|Laboratorio de Running/i })).toBeVisible();

        await page.getByRole("button", { name: /Cataleya/i }).click();
        await expect(page.getByRole("heading", { name: /Cataleya/i })).toBeVisible();
    });
